  ...
  GM_xmlhttpRequest({
    ...
    onload: function (response) {
      ...
      if (status === 200) {
        ...
        forceLoadImage(imgSrc, status); // Pass the status parameter to the function
      }
    },
    onerror: function (error) {
...
(function () {
  ...
  // Load images with retries and replace thumbnails
  const A = document.querySelectorAll('a.fileThumb.image-link');
  const IMG = document.querySelectorAll('.post__image');

  totalImages = A.length; // Set the totalImages count to the number of images on the page

  for (let i = 0; i < A.length; i++) {
    setTimeout(function (index) {
      loadImageWithRetriesAndReplace(IMG[index], A[index], 1);
    }, i * 2500 , i);
  }
