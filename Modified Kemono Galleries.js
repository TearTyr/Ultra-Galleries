...
(function () {
  ...
  function updateImageLoadingStatus() {
    const imageLoadingStatus =
      imageCount === totalImages || imageStatusUpdated === true
  ...
  function updateDownloadStatus() {
    const downloadStatus =
      downloadedCount === totalImages ? `Downloading...` : `Done Downloading!`;
      ...
      GM.xmlHttpRequest({
        ...
        onerror: function (error) {
          if (retryCount < MAX_RETRIES) {
            console.warn(
              `Failed to download image: ${imgSrc}, retrying... (Attempt ${
                retryCount + 1
              })`
            );
            setTimeout(function () {
  ...
  function downloadFile(index) {
    const imgInfo = imageInfo[index];

    GM_download({
      url: imgInfo.url,
      name: imgInfo.name,
      onload: function () {
        console.log("File downloaded successfully:", imgInfo.url);
        downloadedCount++;
        updateDownloadStatus();
      },
      onerror: function (error) {
        console.error("Failed to download file:", imgInfo.url, error);
      },
    });
  }
  ...
  function downloadAllImagesAndVideos() {
    ...
    const imagePromises = Array.from(images).map((img, index) => {
      ...
      const imgName = `${artistName}-${title}-${index}.${extension}`.replace(
        /[\\/:*?"<>|]/g,
        "-"
      );
    ...
    Promise.all([...imagePromises, ...videoPromises])
      .then(() => {
        zip
          ...
          .then((content) => {
            const zipFileName = `${artistName}-${title}.zip`.replace(
              /[\\/:*?"<>|]/g,
              "-"
            );
          ...
          .finally(() => {
            updateStatus(
              `Done Downloading and adding to a zip! Total: ${total}`
            );
          });
      })
      .catch((error) => {
      ...
      .then((response) => {
        ...
        if (status === 200) {
          ...
          if (retryCount <= MAX_RETRIES) {
            ...
            console.log(
              `Retrying image load: ${img.getAttribute("src")} in ${
                delay / 1000
            ...
            console.error(
              `Max retries exceeded for image: ${img.getAttribute("src")}`
            );
  ...
  function loadImages() {
    ...
    const loadImagePromises = Array.from(A).map((a, index) => {
      return new Promise((resolve, reject) => {
        loadImageWithRetriesAndReplace(IMG[index], a, 0);
        resolve();
      });
    });
  ...
  if (parentDiv) {
    for (let i = 0; i < DIV.length; i++) {
      ...
      newDiv.append(
        ...
        createToggleButton(DL, downloadFile),
